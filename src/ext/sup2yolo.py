...
class S2Y:
    ...
    def get_yolo_annotation_info(folder_name, file_name, class_names_array):
        image_path = (
            supervisely_path + "//dataset//" + folder_name + "//img//" + file_name
        )
    ...
    def create_text_file(folder_name, file_name, class_names_array):
        ...
        class_coord_list = S2Y.get_yolo_annotation_info(
...
class Y2S:
    ...
    def create_supervisely_file_structure():
        os.makedirs(
            os.path.dirname(supervisely_path + "//dataset//ann//"), exist_ok=True
        )
        os.makedirs(
            os.path.dirname(supervisely_path + "//dataset//img//"), exist_ok=True
        )
    ...
    def get_supervisely_annotation_info(file_name):
        for image_path in glob.glob(
            os.path.join(yolo_path + "//images//", file_name + ".*")
        ):
            pass

        if not skip_copy:
            copy_path = (
                supervisely_path + "//dataset//img//" + os.path.basename(image_path)
            )
            copyfile(image_path, copy_path)
    ...
    def create_meta_file(class_names_array):
        ...
        for name in class_names_array:
            classes_array.append(
                {"title": name, "shape": "rectangle", "color": "#FF0000"}
            )

        meta_format = {"tags": [], "classes": classes_array}
            ...
            json_format["objects"].append(
                {
                    ...
                    "points": {
                        "exterior": [
                            [class_coord["x1"], class_coord["y1"]],
                            [class_coord["x2"], class_coord["y2"]],
                        ],
                        "interior": [],
                    },
                }
...
def main():
    ...
    if conversion_type == y2s_flag:
        try:
            ...
            print(
                'Error [classes.txt not found] => There should be a folder "yolo" at {0}'.format(
                    yolo_path[:-4]
                )
            )
            exit(1)
        ...
        try:
            ...
            print(
                'Error [meta.json not found] => There should be a folder "supervisely" at {0}'.format(
                    supervisely_path[:-11]
                )
            )
            exit(1)
        ...
        dataset_folders = [
            folder for folder in os.listdir(supervisely_path + "//dataset//")
        ]
